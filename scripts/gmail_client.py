#!/usr/bin/env python3
"""
Gmail API Client - OAuth2 인증 및 API 연결 관리 (발송 권한 포함)
"""
...
CREDS_DIR = Path(os.getenv('GMAIL_CREDS_DIR', str(Path.home() / 'projects/obsidian/.creds')))
        ...
        if creds and creds.expired and creds.refresh_token:
            ...
            if not CREDENTIALS_FILE.exists():
                raise FileNotFoundError(
                    ...
                    "oauth_client.json을 다운로드하여 위 경로에 저장하세요."
                )
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(f"Gmail API 연결 성공! (발송 권한 포함)")
